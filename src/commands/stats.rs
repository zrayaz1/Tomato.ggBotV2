...
use crate::errors::{FetchUserIDError, 
                    ClanInfoFetchError,
                    CreatePeriodEmbedError,
                    CreateMainStatEmbedError,};
...
impl PlayerData {
    pub fn get_period_data(&self, period: Period) -> Option<&TimeFrame> {
        if let Some(recents) = &self.recents {
            match period {
                Period::R24HR => {return Some(&recents.recent24hr);},
                Period::R3DAYS => {return Some(&recents.recent3days);},
                Period::R7DAYS => {return Some(&recents.recent7days);},
                Period::R30DAYS => {return Some(&recents.recent30days);},
                Period::R60DAYS => {return Some(&recents.recent60days);},
                Period::R1000BATTLES => {return Some(&recents.recent1000battles);},
                Period::R100BATTLES => {return Some(&recents.recent100battles);},
            }
        }
        None
    }
...
pub async fn fetch_user_id(input: &str, region: Region) 
...
pub async fn fetch_clan_info(region: &Region, account_id: &u32) 
...
pub async fn generate_period_embed(player_data: &PlayerData, period: Period) 
    ...
    match player_data.get_period_data(period) {
        ...
        None => {return Err(CreatePeriodEmbedError::MissingRecentsError);}
    }
...
pub async fn generate_main_stat_embed(data: &PlayerData) 
    ...
    embed.field("**24 Hours**",
                format!("Battles: `{}`\nWN8: `{}`\nWinRate: `{}%`\nAvgTier: `{}`",
                        ...
                        recents.recent24hr.overall.tier),true);
    ...
    embed.field("**7 Days**",
                format!("Battles: `{}`\nWN8: `{}`\nWinRate: `{}%`\nAvgTier: `{}`",
                        ...
                        recents.recent7days.overall.tier),true);
    ...
    embed.field("**30 Days**",
                format!("Battles: `{}`\nWN8: `{}`\nWinRate: `{}%`\nAvgTier: `{}`",
                        ...
                        recents.recent30days.overall.tier),true);
    ...
    embed.field("**60 Days**",
                format!("Battles: `{}`\nWN8: `{}`\nWinRate: `{}%`\nAvgTier: `{}`",
                        ...
                        recents.recent60days.overall.tier),true);
    ...
    embed.field("**1000 Battles**",
                format!("Battles: `{}`\nWN8: `{}`\nWinRate: `{}%`\nAvgTier: `{}`",
                        ...
                        recents.recent1000battles.overall.tier),true);
    embed.footer(|f| {
...
pub async fn stats(
    ...
    match player_clan_fetch {
        Ok(data) => {all_data.player_clan = data;}
    ...
    match period {
        Some(period) => {
            match generate_period_embed(&all_data, period).await {
                Ok(period_embed) => {embed = period_embed;}
                Err(_) => {
                    embed = CreateEmbed::default()
                    .title("Not in Cache... Please wait").to_owned();
                }
            }
        }

        None => {
            match generate_main_stat_embed(&all_data).await {
    ...
    match &all_data.player_clan {
        Some(clan) => {
            ...
            match clan_data {
                Ok(data) => {all_data.clan = Some(data);}
            ...
            {   
                match mci.data.component_type {
                    ComponentType::SelectMenu => {
                        ...
                        println!("{}",period);
                        embed = generate_period_embed(&all_data, Period::from_str(period).unwrap()).await.unwrap();
                        if all_data.is_in_clan {
                            message.edit(ctx, |f| {f.embed(|f| {f.clone_from(&embed);f})
                                .components(|c| {
                                    c.create_action_row(|ar| {
